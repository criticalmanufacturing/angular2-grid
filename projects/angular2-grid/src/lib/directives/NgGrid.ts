...
export class NgGrid implements OnInit, DoCheck, OnDestroy {
  ...
  @Output() public onItemChange: EventEmitter<Array<NgGridItemEvent>> = new EventEmitter<
    Array<NgGridItemEvent>
  >();
    ...
    if (this._itemFixDirection === 'vertical') {
      fixLoop: for (; newPos.col <= maxRow; ) {
        ...
        newPos.col = Math.max(
          ...
          Math.min.apply(
            Math,
            itemsInPath.map((item) => item.col + dims.x),
          ),
        );
        newPos.row = 1;
      }
      ...
      fixLoop: for (; newPos.row <= maxRow; ) {
        ...
        newPos.row = Math.max(
          ...
          Math.min.apply(
            Math,
            itemsInPath.map((item) => item.row + dims.y),
          ),
        );
        newPos.col = 1;
      }
  ...
  private _getItemFromPosition(position: NgGridRawPosition): NgGridItem {
    return Array.from(
      this._itemsInGrid,
      (itemId: string) => this._items.get(itemId) as NgGridItem,
  ...
  private _createPlaceholder(item: NgGridItem): void {
    ...
    var componentRef: ComponentRef<NgGridPlaceholder> =
      item.containerRef.createComponent(NgGridPlaceholder);
  ...
  private _enableMouseListeners(): void {
    // add event listeners outside of angular zone
    this._ngZone.runOutsideAngular(() => {
      const documentMousemoveSubs = this._documentMousemove$.subscribe((e: MouseEvent) =>
        this.mouseMoveEventHandler(e),
      );
      const documentMouseupSubs = this._documentMouseup$.subscribe((e: MouseEvent) =>
        this.mouseUpEventHandler(e),
      );
